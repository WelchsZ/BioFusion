...
from DE.configs import hyperparameters
from DE.configs import global_config
...
class Space_Regulizer:
    ...
    def ball_holder_loss_lazy(self, new_G, num_of_sampled_latents, w_batch, use_wandb=False):
        ...
        #                                    truncation_psi=0.5)
        z_samples = np.random.randn(num_of_sampled_latents, self.original_G.g_mapping.latent_size)
        w_samples = self.original_G.g_mapping(torch.from_numpy(z_samples.astype(np.float32)).to(global_config.device))
        ...
        for w_code in territory_indicator_ws:
            ...
            new_img = new_G.g_synthesis(w_code, depth=6, alpha=1)
            with torch.no_grad():
                # old_img = self.original_G.synthesis(w_code, noise_mode='none', force_fp32=True)
                old_img = self.original_G.g_synthesis(w_code, depth=6, alpha=1)
