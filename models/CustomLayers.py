...
class EqualizedLinear(nn.Module):
    ...
    def forward(self, x):
        bias = self.bias
        # if bias.device == torch.device('cpu'):
        ...
        return F.linear(x, weight * self.w_mul, bias)
...
class EqualizedConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if not have_convolution and intermediate is None:
            return F.conv2d(x, weight * self.w_mul, bias, padding=self.kernel_size // 2)
        elif not have_convolution:
            x = F.conv2d(x, weight * self.w_mul, None, padding=self.kernel_size // 2)
...
class NoiseLayer(nn.Module):
    ...
    def forward(self, x, noise=None):
        if noise is None and self.noise is None:
            ...
            noise = self.noise
        x = x + self.weight.view(1, -1, 1, 1).to(device) * noise
        return x
